...
namespace E___Purchase.Areas.Customer.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Details(ShoppingCart shoppingCart)
        {
            ...
            var userId = claimsIdentity.FindFirst(ClaimTypes.NameIdentifier).Value;//PE67T8

            Guid userIdGuid = Guid.Parse(userId);
            shoppingCart.ApplicationUseId = userIdGuid;

            var cartFrmDb = _unitOfWork.ShoppingCart.Get(u => u.ApplicationUseId == userIdGuid && u.ProductId == shoppingCart.ProductId);
            if (cartFrmDb !=null)
            {
